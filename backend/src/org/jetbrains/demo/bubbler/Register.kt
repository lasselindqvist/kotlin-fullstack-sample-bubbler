...
fun Route.register(dao: BubblerStorage, hashFunction: (String) -> String) {
    post<Register> { form ->
        ...
        if (user != null) {
            ...
            if (form.password.length < 6) {
                ...
                val newUser = User(form.userId, form.email, form.phoneNumber, form.displayName, hash)
                ...
                try {
                    ...
                    } else if (dao.userByPhoneNumber(form.phoneNumber) != null) {
                        call.respond(LoginResponse(error = "User with the following phone number ${form.phoneNumber} is already registered"))
                    } else {
