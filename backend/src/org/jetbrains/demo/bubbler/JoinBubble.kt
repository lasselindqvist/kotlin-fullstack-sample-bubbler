...
fun Route.joinBubble(dao: BubblerStorage, hashFunction: (String) -> String) {
    get<JoinBubble> {
        ...
        if (user == null) {
            ...
            if (!call.verifyCode(date, user, code, hashFunction)) {
                ...
                call.respond(JoinBubbleResponse(user, null, ""))
            }
        }
    }
    post<JoinBubble> {
        ...
        if (user == null) {
            ...
            if (bubble == null) {
                ...
                dao.joinBubble(user.userId, bubbleId)
                call.respond(JoinBubbleResponse(user, newBubble, ""))
            } else {
                dao.joinBubble(user.userId, bubble.bubbleId)
                call.respond(JoinBubbleResponse(user, bubble, ""))
            }
        }
    }
}
