...
class NavBarComponent : ReactDOMComponent<NavBarComponent.NavBarHandlerProps, NavBarComponent.NavBarState>() {
    ...
    override fun ReactDOMBuilder.render() {
        ...
        ul(classes = "nav-list") {
            ...
            if (user != null) {
                ...
                navItem("Join Bubbles") {
                    joinBubble()
                }
                navItem("Leave Bubbles") {
                    leaveBubble()
                }
                navItem("Show Bubbles") {
                    showBubble()
                }
            } else {
                navItem("Sign up") {
    ...
    private fun joinBubble() {
        props.handler(MainView.JoinBubble)
    }

    private fun leaveBubble() {
        props.handler(MainView.LeaveBubble)
    }

    private fun showBubble() {
        props.handler(MainView.ShowBubble)
    }
    ...
    class NavBarState(var bubbleName: String, var newMessages: Polling.NewMessages = Polling.NewMessages.None) : RState
